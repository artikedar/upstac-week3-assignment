...
public class TestRequestFlowService {
    ...
    public List<TestRequest> findByTester(User tester) {
        List<TestRequestFlow> testRequestFlows = testRequestFlowRepository.findTestRequestFlowByChangedBy(tester);
        return testRequestFlows.stream().map(TestRequestFlow:: getRequest).collect(Collectors.toList());
    }
}
